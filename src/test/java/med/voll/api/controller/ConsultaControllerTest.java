...
class ConsultaControllerTest {
    ...
    @Test
    @DisplayName("Deve devolver codigo http 400 quando informações estão inválidas")
    @WithMockUser
    ...
    void agendar_cenario2() throws Exception {
        ...
        var dadosDetalhamento = new DadosDetalhamentoConsultaDTO(null, 2l,5l,data,true);
        var dadosAgendamento = new DadosAgendamentoConsultaDTO(2l,5l, data, especialidade);

        when(agendaDeConsultas.agendar(any())).thenReturn(dadosDetalhamento);

        var response = mvc.perform(post("/consultas")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(dadosAgendamentoConsultaDTOJson
                                .write(dadosAgendamento).getJson()))
                .andReturn().getResponse();

        var jsonEsperado = dadosDetalhamentoConsultaDTOJson
